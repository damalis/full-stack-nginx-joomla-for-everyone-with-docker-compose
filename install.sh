...
echo "======================================================================="
echo "|                                                                     |"
echo "|      full-stack-nginx-joomla-for-everyone-with-docker-compose       |"
echo "|                        by Erdal ALTIN                               |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
then
	...
	sudo -- sh -c -e "grep -qxF '127.0.0.1  mail.$domain_name' /etc/hosts || echo '127.0.0.1  mail.$domain_name' >> /etc/hosts"																														
else
	domain_name=""
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing Joomla and the other services
	docker compose up -d & export pid=$!
	echo "Joomla and the other services installing proceeding..."
	echo ""
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo ""
		echo "Error! could not installed Joomla and the other services with docker compose" >&2
		echo ""
		exit 1
	fi
else
