...
##### # backup exclude joomla, backups folders in ${DIRECTORY_PATH}

```
00 01 * * * tar -czvf ${DIRECTORY_PATH}/backups/'backup-example.com-'$(date +"\%Y-\%m-\%dT\%H-\%M-\%S")'.tar.gz' --exclude='joomla/cache' --exclude='joomla/tmp' --exclude='joomla/includes' --exclude='backups' ${DIRECTORY_PATH}
```
